$(document).ready(function (){
	...
	if ( $(clickedobject).hasClass('md-trigger') ) {
	    ...
	    } else if (puzzle_action === 'stop') {
		stopActionClicked(clickedobject);
	...
	if (!solver_running) {
	    ...
	    $(actionElement).attr('data-action','stop');
	}
    };


    function stopActionClicked(actionElement) {
	if (solver_running) {
	    ...
	    $(actionElement).attr('data-action','solve');
	}
    };
	...
	new_clues_row = $(new_clues_row).removeClass("column-totals-row").addClass("clue-index-"+new_clue_index);
	$(new_clues_row).find("li span").each( function(index, value) {
		var column_index=$(value).data('column');
		$(value).replaceWith("<span class='no-column-clue column-"+column_index+" clue-index-"+new_clue_index+"' data-clue-index='"+new_clue_index+"' data-column='"+column_index+"'>&nbsp;</span>");
	});
	...
	$.each(column_clues, function(index, value) {
		...
		var selector = ".column-clues-row.clue-index-"+add_clue_index+" span.column"+column;
		//console.log(index + ':' + clues_count + ':' + add_clue_index + ':' + selector);
		//Add a plus sign above the clue
		if (add_clue_index === new_clue_index) {
		    $(new_clues_row).find("span.column-"+column).replaceWith("<span class='add-column-clue  column-"+column+"'clue-index-"+add_clue_index+"' data-column='"+column+" data-clue-index='"+add_clue_index+"'>+</span>");
		} else {
		    $(".column-clues-row.clue-index-"+add_clue_index+" span.column-"+column).replaceWith("<span class='add-column-clue column-"+column+"' data-column='"+column+"'>+</span>");
		}
	});
    };
	...
	$(puzzle_rows).each( function(index, value) {
		...
		$(row_clues).find("ul").prepend("<li><span class='add-row-clue row-"+row_number+" clue-index-"+new_clue_index+"' data-row='"+row_number+"' data-clue-index='"+new_clue_index+"'>+</span></li>");
	    } else {
		$(row_clues).find("ul").prepend("<li><span class='no-row-clue row-"+row_number+" clue-index-"+new_clue_index+"' data-row='"+row_number+"' data-clue-index='"+new_clue_index+"'>&nbsp</span></li>");
		var add_clue_index = row_clue_count + 1;
		$(row_clues).find("ul li span.clue-index-"+add_clue_index).replaceWith("<span class='add-row-clue row-"+row_number+" clue-index-"+add_clue_index+"' data-row='"+row_number+"' data-clue-index='"+add_clue_index+"'>+</span>");
	    }
	...
	$.each(plus_cells, function(index, value) {
		...
		var column=$(value).data('column');
		$(value).replaceWith("<span class='no-column-clue column-"+column+" clue-index-"+clue_index+"' data-column='"+column+"' data-clue-index='"+clue_index+"'>&nbsp;</span>");
	});
    };
	...
	$.each(plus_cells, function(index, value) {
		...
		$(value).replaceWith("<span class='no-row-clue row-"+row+" clue-index-"+clue_index+"' data-row='"+row+"' data-clue-index='"+clue_index+"'>&nbsp;</span>");
	});
	var dynamic_styles = $("#dynamic-styles");
	//console.log('dynamic styles ');
