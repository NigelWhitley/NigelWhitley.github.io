...
import {SudokuGroup} from "./SudokuGroup.js";
	...
	// a unit (block, row or column)
		...
		constructor (originalCells) {
			this.init(originalCells);
		}

		init = function (originalCells) {
		...
		removeValue = function (cellIndex, value) {
			const values = this.workingSet[cellIndex].values;
		...
		removeValueFromWorkingSet = function (value) {
			for ( let  cellIndex = 0; cellIndex < this.workingSet.length; cellIndex++ ) {
		...
		removePositionFromWorkingSet = function (position) {
			for ( let  cellIndex = this.workingSet.length - 1; cellIndex >= 0; cellIndex-- ) {
		...
		buildListForNakedSearch = function () {
			for ( let  cellIndex = 0; cellIndex < this.possibleSet.length; cellIndex++ ) {
				const  cellPos = this.possibleSet[cellIndex];
				const  cell = SudokuGlobal.puzzle.getCell(this.possibleSet[cellIndex]);
				// Ignore any cell which has a fixed value
				if ( ! cell.isFixedValue() ) {
                    // Make a copy of the values in the cell
					const  values = cell.getPossibleValues().slice();
		...
		sortListForNakedSearch = function () {
		...
		buildNakedGroups = function () {
			...
			// or a group is found.
			let initialGroupSizeLimit = 1;
			let groupSizeLimit = initialGroupSizeLimit;
			let sizeOfLargestGroup = 0;
			let workingGroup = new SudokuGroup();
			let workingIndex = 0;
			...
			do {
				...
				if ( ( workingGroup.countsMatch() ) && ( buildStack.length !== this.workingSet.length ) ) {
					...
					const groupSize = workingGroup.getValueCount();
					if ( groupSize > sizeOfLargestGroup ) {
						sizeOfLargestGroup = groupSize;
					}
					this.groups.push(workingGroup);
					do {
						const removeIndex = buildStack.pop();
						this.workingSet.splice(removeIndex, 1);
					} while ( buildStack.length > 0 );
					const valuesAsArray = workingGroup.getValuesAsArray();
					for ( let  valueIndex = 0; valueIndex < valuesAsArray.length; valueIndex++ ) {
						const removeValue = valuesAsArray[valueIndex];
					...
					if ( ( workingGroup.valueCount === this.workingSet.length ) || 
						...
						for ( let  rebuildIndex = 0; rebuildIndex < buildStack.length; rebuildIndex++ ) {
			...
			if ( this.workingSet.length < sizeOfLargestGroup ) {
				workingGroup = new SudokuGroup();
				//debugFramework.showMessage("bNG rebuilding " + workingIndex, "append");
				for ( let  rebuildIndex = 0; rebuildIndex < this.workingSet.length; rebuildIndex++ ) {
					workingGroup.addValues(this.workingSet[rebuildIndex].values);
					workingGroup.addPosition(this.workingSet[rebuildIndex].position);
				}
		...
		static withoutFixedValues = function ( originalPositions ) {
			// We want to build a new array which excludes any cell with a
			// fixed value. Note that we don't change the original array.
			let remainingPositions = new Array();
			for ( let  cellIndex = 0; cellIndex < originalPositions.length; cellIndex++ ) {
				const  cellPos = originalPositions[cellIndex];
				const  cell = SudokuGlobal.puzzle.getCell(cellPos);
				// Ignore any cell which has a fixed value
				if ( ! cell.isFixedValue() ) {
		...
		static withoutGroup = function ( originalPositions, group ) {
			// We want to build a new array which excludes any cell in the 
			// group. Note that we don't change the original array.
			const  groupPositions = group.getPositionAsArray();
			const  remainingPositions = SudokuCellSet.withoutPositions ( originalPositions, groupPositions );
			return remainingPositions;
		}

		static withoutPositions = function ( originalPositions, removePositions ) {
			// We want to build a new array which excludes any cell in the 
			...
			for ( let  cellIndex = 0; cellIndex < originalPositions.length; cellIndex++ ) {
				const  cellPos = originalPositions[cellIndex];
				let  keepCell = true;
				for ( let  removeIndex = 0; removeIndex < removePositions.length; removeIndex++ ) {
					const  removePos = removePositions[removeIndex];
